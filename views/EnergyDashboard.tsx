...
export const EnergyDashboard: React.FC<EnergyDashboardProps> = ({ 
  ...
  const scores = useMemo(() => {
    ...
    // Apply modifiers based on activities
    const breaks = energy.physical.breaks || 0;
    const meetingMinutes = energy.social.meetingMinutes || (energy.social.meetings || 0) * 60;
    const focusBlocks = energy.mental.focusBlocks || 0;

    // Breaks restore energy
    physical = Math.min(100, physical + (breaks * 10));
    mental = Math.min(100, mental + (breaks * 15));
    emotional = Math.min(100, emotional + (breaks * 5));

    // Focus blocks drain mental energy
    mental = Math.max(0, mental - (focusBlocks * 15));
    physical = Math.max(0, physical - (focusBlocks * 5));

    // Meetings drain social and mental energy proportionally
    // 60 minutes = 15 social, 10 mental, 5 emotional
    const meetingHours = meetingMinutes / 60;
    social = Math.max(0, social - (meetingHours * 15));
    mental = Math.max(0, mental - (meetingHours * 10));
    emotional = Math.max(0, emotional - (meetingHours * 5));
