...
const App: React.FC = () => {
  ...
  const handleTaskComplete = (type: 'FOCUS' | 'BREAK' | 'MEETING', durationInSeconds?: number) => {
      const today = new Date().toISOString().split('T')[0];
      const existingLog = logs.find(l => l.date === today);
      
      if (existingLog && existingLog.energy) {
          const updatedEnergy = { ...existingLog.energy };
          
          if (type === 'FOCUS') {
              updatedEnergy.mental.focusBlocks = (updatedEnergy.mental.focusBlocks || 0) + 1;
          } else if (type === 'BREAK') {
              updatedEnergy.physical.breaks = (updatedEnergy.physical.breaks || 0) + 1;
          } else if (type === 'MEETING') {
              updatedEnergy.social.meetings = (updatedEnergy.social.meetings || 0) + 1;
              const minutes = durationInSeconds ? Math.round(durationInSeconds / 60) : 60;
              updatedEnergy.social.meetingMinutes = (updatedEnergy.social.meetingMinutes || 0) + minutes;
          }
          
          handleEnergyUpdate({ energy: updatedEnergy });
      }
      setView('dashboard');
  };
  ...
  return (
    <>
        {view === 'morning-checkin' ? (
            ...
            <FocusTimer 
                mode="FOCUS" 
                onComplete={() => handleTaskComplete('FOCUS')} 
                onCancel={() => setView('dashboard')} 
            />
        ) : view === 'break-mode' ? (
            <FocusTimer 
                mode="BREAK" 
                onComplete={() => handleTaskComplete('BREAK')} 
                onCancel={() => setView('dashboard')} 
            />
        ) : view === 'meeting-mode' ? (
            <MeetingTimer 
                onComplete={(duration) => handleTaskComplete('MEETING', duration)} 
                onCancel={() => setView('dashboard')} 
            />
